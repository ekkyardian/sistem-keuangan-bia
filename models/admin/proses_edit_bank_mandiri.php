...
if($pict == '') {
    $bankmandiri->edit("UPDATE tb_transaksi SET no_voucher='$no_voucher', tanggal='$tanggal', deskripsi='$deskripsi', jenis='$jenis', jumlah='$jumlah', lokasi_dana='$lokasi_dana', id_kategori='$id_kategori' WHERE id_transaksi='$id_transaksi'");
    echo "<script>window.location='?page=bank_mandiri';</script>";
} else {
    $kwitansi_awal = $bankmandiri->tampil($id_transaksi)->fetch_object()->kwitansi_pendukung;
    unlink("../../assets/img/kwitansi/".$kwitansi_awal);
    
    $upload = move_uploaded_file($sumber, "../../assets/img/kwitansi/".$kwitansi_pendukung);
    if($upload) {
        $bankmandiri->edit("UPDATE tb_transaksi SET no_voucher='$no_voucher', tanggal='$tanggal', deskripsi='$deskripsi', jenis='$jenis', jumlah='$jumlah', lokasi_dana='$lokasi_dana', id_kategori='$id_kategori', kwitansi_pendukung='$kwitansi_pendukung' WHERE id_transaksi='$id_transaksi'");
    echo "<script>window.location='?page=bank_mandiri';</script>";
    } else {
